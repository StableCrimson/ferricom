...
impl Map for NROM {
  ...
  fn map_read(&self, addr: u16) -> MappedRead {
    match addr {
      CHR_ROM_BANK_START..=CHR_ROM_BANK_END => MappedRead::Chr(addr),
      PRG_RAM_START..=PRG_RAM_END => MappedRead::PrgRAM(addr & 0x1FFF),
      PRG_ROM_BANK_1_START..=PRG_ROM_BANK_1_END => MappedRead::PrgROM(addr & 0x3FFF),
      PRG_ROM_BANK_2_START..=PRG_ROM_BANK_2_END => {
        let mirror_mask = if self.mirror_prg_rom {0x3FFF} else {0x7FFF};
        MappedRead::PrgROM(addr & mirror_mask) 
      }
      _ => MappedRead::None
    }
  }
  ...
  fn map_write(&self, addr: u16, data: u8) -> MappedWrite {
    match addr {
      CHR_ROM_BANK_START..=CHR_ROM_BANK_END => MappedWrite::Chr(addr, data),
      PRG_RAM_START..=PRG_RAM_END => MappedWrite::PrgRAM(addr & 0x1FFF, data),
      _ => MappedWrite::None,
    }
  }
}